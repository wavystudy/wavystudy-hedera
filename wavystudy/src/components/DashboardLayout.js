...
import navicationIcon from '../assets/wavy-study-logo.svg'
      ...
      <div className="h-screen flex overflow-hidden">
         <div className="bottomBar bg-white flex flex-col">
           <div className="bottomIcon border-b border-b-gray-200 bg-gray-100 flex justify-center" >
              <img src={navicationIcon} className="h-full w-20"/>
           </div>
           {ROUTES.map(({id, IconComponent, title, route }) => {
              ...
              return (
               ...
               >
                  <IconComponent className={`h-6 w-6 ${active && 'text-emerald-400'}`} />
                  <p className={`text-md ${active && 'text-emerald-400'}`}>{title}</p>
               </div>
              );
            })}
            ...
            <div
               className="aspect-square w-full border-b border-b-gray-200 flex flex-col items-center justify-center hover:cursor-pointer"
               onClick={() =>{Cookies.remove("login");Cookies.remove("userid");    window.localStorage.removeItem("loggedin");window.location.href="/";} }
            >
               <ArrowCircleLeftIcon className="h-6 w-6" />
               <p className={`text-md`}>Log out</p>
            </div>
         </div>
         <div style={{"background":"#F8F8F8"}} className="flex-1 overflow-y-auto scroll-smooth">
