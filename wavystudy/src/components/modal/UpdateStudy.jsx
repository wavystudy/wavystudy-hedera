...
export default function UpdateStudyModal({
    ...
    async function UpdateStudyHandle(e) {
        ...
        try {
			...
			.addUint256(Number(id))
			.addString(image.value)
			.addString(title.value)
			.addString(description.value)
			.addUint256(( parseInt(budget.value) * 1e8));

            await sendTransaction( "UpdateStudy",params);
    ...
    async function LoadData() {
        if (typeof window?.contract !== 'undefined' ) {
            try {
                let study_element = await ReadContractByQuery( ("_studyMap"), [parseInt(id)]);
                var newStudy = {
                ...
                <Form onSubmit={UpdateStudyHandle}>
                    ...
                    <Form.Group className="mb-3 grid" controlId="formGroupName">
                        ...
                        <div className="input-group">
                            ...
                            <input required name="budget" placeholder="Budget"  step={"0.0001"} id="updatebudget" type='number' className="w-24 text-black pr-2 border-gray-400 border pl-2" />
                        </div>
                    </Form.Group>
                    <div className="d-grid">
                        <Button name="updateBTN" type='submit' style={{ 'display': 'flex' }} className='w-[128px] h-12 flex justify-center items-center'  >
