...
export default function CreateStudyModal({show, onHide}) {
	...
	async function CreateStudyHandle(e) {
		...
		try {
			...
			.addUint256(Number(Cookies.get("userid")))
			.addString(image.value)
			.addString(title.value)
			.addString(description.value)
			.addString(JSON.stringify(permissionData))
			.addUint256(0)
			.addUint256(0)
			.addUint256(( parseInt(budget.value) * 1e8));
			await sendTransaction( "CreateStudy", params)
				...
				<Form onSubmit={CreateStudyHandle}>
					...
					<Form.Group className="mb-3 grid" controlId="formGroupName">
						...
						<div className="input-group">
							...
							<input required name="budget" placeholder="Budget"  step={"0.0001"} id="budget" type="number" className="w-24 text-black pr-2 border-gray-400 border pl-2" />
						</div>
					</Form.Group>
					<Form.Group className="mb-3 grid" controlId="formGroupName">
					...
					<div className="d-grid">
						<Button name="createBTN" type="submit" style={{"display": "flex"}} className="w-[128px] h-12 flex justify-center items-center" >
