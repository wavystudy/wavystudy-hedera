...
export default function UpdateSurveyModal({
    ...
    async function UpdateSurveyHandle(e) {
        ...
        try {
			...
			.addUint256(( parseInt(reward.value) * 1e8));


            await sendTransaction( "UpdateSurvey",params);
    ...
    async function LoadData() {
        if (typeof window?.contract !== 'undefined' ) {
            try {
                let survey_element = await ReadContractByQuery( ("_surveyMap"), [parseInt(id)]);
                var new_survey = {
                ...
                <Form onSubmit={UpdateSurveyHandle}>
                    ...
                    <Form.Group className="mb-3 grid" controlId="formGroupName">
                        ...
                        <div className="input-group">
                            ...
                            <input required name="reward" placeholder="Reward"  step={"0.0001"} id="reward" type='number' className="w-24 text-black pr-2 border-gray-400 border pl-2" />
                        </div>
                    </Form.Group>
