...
function SurveyDetails() {
	...
	async function AddCategory(e, index) {
		...
		.addString(categoryname.value)
		.addString(categoryimagelink.value);

		await sendTransaction( "CreateSurveyCategory", paramsTrans);
	...
	async function SaveSection(e) {
		...
		try {
			let paramsTrans = new ContractFunctionParameters()
			.addUint256(Number(SURVEY_DATA.id))
			.addString(JSON.stringify(sectionsdata));
	
			await sendTransaction( "CreateOrSaveSections",paramsTrans);
		} catch (error) {
	...
	async function LoadDataStudy() {
		if (contract !== null ) {
			...
			let study_element = await ReadContractByQuery( ("_studyMap"), [parseInt(location.state.studyID)]);
			var newStudy = {
	...
	async function LoadSurveyData() {
		if (contract !== null ) {
			...
			let survey_element = await ReadContractByQuery( ("_surveyMap"), [parseInt(params.id)]);
			var new_survey = {
	...
	async function LoadDataSections() {
		if (contract !== null ) {
			...
			try {
				let SectionsInfo =  JSON.parse(await ReadContractByQuery( ("_sectionsMap"), [parseInt(0)]));
	...
	async function LoadDataCategories() {
		try {
			...
			const totalSurveyCategory = await ReadContractByQuery( ("_SurveyCategoryIds"))    
		
				for (let i = 0; i < Number(totalSurveyCategory); i++) {
					const element = await ReadContractByQuery( ("_categoryMap"), [parseInt(i)]);
					setdataCategory((prevState) => [
						...prevState,
						{
							value: element.name,
						...
						<div className="container-Survey-Template">
							...
							<div className="Next-Button">
								<div className="flex">
									...
									<button id="surveyDelete" className="flex w-[62px] h-10 border border-gray-400  rounded-md justify-center items-center mx-1 hover:bg-white">
					...
					<>
						<div className="flex p-6">
							...
							<div className="flex">
								...
								<button id="surveyDelete" className="flex w-[52px] h-10 border border-gray-400  rounded-md justify-center items-center mx-1 hover:bg-white">
									<TrashIcon className="w-5 h-5 text-gray-400" />
								</button>
								<button onClick={addSection} className="h-10 rounded-md shadow-md bg-black text-white flex py-2 px-4 items-center hover:bg-gray-700 hover:text-gray-500">
									<PlusSmIcon className="w-5 h-5 " />
							...
							<div key={item.id} htmlFor={item.id} className="bg-white border border-gray-400 rounded-lg flex flex-col mt-4">
								<div className="bg-gray-100 py-4 px-6 border-b border-b-gray-400">
									<div className="flex mb-4 items-center">
										...
										<button className="flex w-[52px] h-10 border border-gray-400  rounded-md justify-center items-center ml-1">
						...
						return (
							<div key={item.id} htmlFor={item.id} className="bg-white border border-gray-400 rounded-lg flex flex-col mt-4 overflow-hidden">
								...
								{sectionsQuestionsdata
									.filter((eq) => eq.section_id === item.id)
									.map((item, index) => {
										return (
