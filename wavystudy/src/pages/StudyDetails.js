...
function StudyDetails() {
	...
	async function UpdateAgesHandle(event) {
		...
		.addUint256(Number(params.id))
		.addString(JSON.stringify(agesData));

		await sendTransaction( "UpdateStudyAges",paramsTrans);
	...
	async function UpdateStudyTitleHandle() {
		...
		let paramsTrans = new ContractFunctionParameters()
		.addUint256(Number(params.id))
		.addString(JSON.stringify(studyTitle.ages_ans));
		await sendTransaction( "CreateOrSaveStudyTitle", paramsTrans);
	...
	async function UpdateAudiences(event) {
		...
		let paramsTrans = new ContractFunctionParameters()
		.addUint256(parseInt(params.id))
		.addString(JSON.stringify(createdObject));

		await sendTransaction( "UpdateAudience", paramsTrans);
		EnableButton("audienceSave");

	}
	async function UpdateRewarads(event) {
		event.preventDefault();
		const { rewardselect, reward_Price, totalspendlimit } = event.target;

		DisableButton("rewardsSave");

		try {
			let paramsTrans = new ContractFunctionParameters()
			.addUint256(Number(parseInt(params.id)))
			.addString(rewardselect.value)
			.addUint256((Number(reward_Price.value) * 1e8))
			.addUint256((parseInt(totalspendlimit.value)* 1e8));

			await sendTransaction( "UpdateReward", paramsTrans);
		} catch (error) {
			console.error(error);
		}
	...
	async function LoadStudyData() {
		...
		let allAudiences = [];
		try {
			allAudiences = JSON.parse(await ReadContractByQuery( ("_studyAudienceMap"), [Number(params.id)]));
		} catch (e) {
			allAudiences = [];
		}
	...
	async function LoadData() {
		...
		if (contract !== null ) {
			setSTUDY_DATA({});
			let study_element = await ReadContractByQuery( ("_studyMap"), [Number(params.id)]);


			let allAges = [];
			try {
				let ages = await ReadContractByQuery( ("_studyAges"), [Number(params.id)]);

				if (ages !== '')
					allAges = JSON.parse(ages);
			} catch (e) { }
			setAgesData(allAges)
			...
			try {
				let titles = await ReadContractByQuery( ("_studyTitles"), [Number(params.id)]);
				if (titles !== "")
					allTitles.ages_ans = JSON.parse(titles);
			} catch (e) { }
			...
			var newStudy = {
				...
				reward_price: window.ParseBigNum(study_element.reward_price),
				total_spending_limit: window.ParseBigNum(study_element.total_spending_limit) 
			};
	...
	async function LoadDataSurvey(contributes = null) {
		if (contract !== null ) {
			if (!LoadingSurvey) {
				...
				let survey_data = []
				setData([]);
				const totalSurveys = await ReadContractByQuery( ("_SurveyIds"));

				try {
					for (let i = 0; i < Number(totalSurveys); i++) {
						let survey_element = await ReadContractByQuery( ("_surveyMap"), [i]);
	...
	async function LoadDataContributors() {
		if (contract !== null ) {
			...
			const totalOngoing = await ReadContractByQuery( ("_OngoingIds"));

			for (let i = 0; i < Number(totalOngoing); i++) {
				let element = await ReadContractByQuery( ("_ongoingMap"), [parseInt(i)]);
				let user_element = await ReadContractByQuery( ("getUserDetails"), [Number(element.user_id)]);
				let fhir_element = await ReadContractByQuery( ("_fhirMap"), [Number(user_element[6])]);
					...
					return (
						<div key={index}>
							...
							<button key={id} onClick={() => setTabIndex(index)} className={`flex items-center h-14 p-4 ${ACTIVE ? "bg-gray-100" : "bg-white"}`}>
								<p className={`${ACTIVE ? "" : "text-black"} font-medium`}>{title}</p>
							</button>
										...
										return (
											<div className="flex mb-2 gap-5 items-center" key={id}>
												...
												<div className="flex">
													...
													<button onClick={() => { duplicateElementFromElementBYIndex(index, "age") }} className="flex w-[52px] h-10 border border-gray-400  rounded-md justify-center items-center ml-1 hover:bg-white">
									...
									{data.map(({ id, name, description, reward, submission, date, image }, index) => {
										...
										return (
											<tr key={id} className={`border-b-gray-400 ${!IS_LAST ? "border-b" : "border-0"}`}>
												...
												<td className="py-3 px-3" style={{ minWidth: "8rem" }}>{`${reward} HBAR`}</td>
