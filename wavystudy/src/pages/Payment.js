...
function Payment() {
	...
	const { contract, signerAddress, sendTransaction,ReadContractByQuery } = useContract();
	...
	async function fetchData() {
		if (contract) {
			...
			let userdetails = await ReadContractByQuery( ("getUserDetails"),[Number(userid)]);
	...
	async function UpdatePayments(e) {
		...
		try {
			...
			// await sendTransaction(signerAddress, "UpdatePrivatekey",[parseInt(userid), privatekey.value]);
